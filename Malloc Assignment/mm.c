...
size_t currentBlockSize = GET_SIZE(HDRP(bp));
   ...
   if(GET_ALLOC(HDRP(NEXT_BLKP(bp))) ==1){
        ...
        currentBlockSize += GET_SIZE(HDRP(NEXT_BLKP(bp)));
...
// previous 
   char  *hp = heap_listp;
    ...
    if(GET_ALLOC(HDRP(hp)) == 1){
    	currentBlockSize += GET_SIZE(HDRP(hp));
        bp = hp;
    	PUT(HDRP(bp), PACK(currentBlockSize,1));
    }
...
void *mm_realloc(void *ptr, size_t size)
{
	...
	void *ptrNew  = mm_malloc(size);  // pointer to the new block
    ...
    // if the old size is greater than new size it will simply reduce the allocated memory 
  ...
  if(oldSize >= size){
     ...
     return ptrNew;
  	 ...
  	 // this is the one I am doing return ptr;
